...
if workItem.wait(timeout: .now() + 3.0) == .timedOut {
  print("Work item is taking longer than 3 seconds to finish.")
} else {
  print("Work item finished before the 3 second timer.")
}
...
func whichQueue(workItem: String) {
  ...
  case .mainQ:
    print(">> \(workItem) ran on the mainQueue")
  case .userQ:
    print(">> \(workItem) ran on the userQueue")
  case .none:
    break
  }
}
