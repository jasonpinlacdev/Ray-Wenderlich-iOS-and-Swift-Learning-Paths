...
appendQueue.maxConcurrentOperationCount = 1 //this makes the operation queue execute operations serially
...
for image in images {
  ...
  tiltShiftOperation.completionBlock = {
    guard let outputImage = tiltShiftOperation.outputImage else { return }
    appendQueue.addOperation {
      filteredImages.append(outputImage)
    }
  }
  
  filterQueue.addOperation(tiltShiftOperation)
}
