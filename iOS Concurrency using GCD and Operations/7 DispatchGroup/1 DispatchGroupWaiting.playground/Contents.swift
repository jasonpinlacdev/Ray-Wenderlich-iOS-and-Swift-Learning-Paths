...
group.notify(queue: DispatchQueue.global()) {
  ...
  print("Group of tasks completed.")
...
// group.wait blocks the calling thread
// the if condition becomes true after 5 seconds
// if the group tasks complete before the wait times out. the else closure executes
if group.wait(timeout: .now() + 5.0) == .timedOut {
  print("I got tired of waiting...")
} else {
  print("Ya! all the tasks in the group did complete!")
}
